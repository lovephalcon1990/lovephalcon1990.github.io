...
class Redis
{
	...
	public function getIncreaseId($appKey, $init_id = 1000)
    {
        if (empty($appKey))
        {
            return false;
        ...
        if ($this->_redis->exists($main_key))
        {
            ...
            if (empty($inc))
            {
                Log::debug("redis::incr() failed. Error: ".$this->_redis->getLastError());
                return false;
            }
            return $inc;
        }
        ...
        else
        {
            ...
            if ($init == false)
            {
                Log::debug("redis::set() failed. Error: ".$this->_redis->getLastError());
                return false;
            }
            else
            {
    ...
    function connect()
    {
        try
        {
            ...
            if(!empty($this->config['seria'])){
	            $this->_redis->setOption(\Redis::OPT_SERIALIZER, \Redis::SERIALIZER_PHP );
            }
        }
        catch (\RedisException $e)
        {
            Log::debug(__CLASS__ . " Swoole Redis Exception" . var_export($e, 1));
            return false;
        }
    }
    ...
    function __call($method, $args = array())
    {
        ...
        while (1)
        {
            ...
            catch (\RedisException $e)
            {
                ...
                Log::debug(__CLASS__ . " [" . posix_getpid() . "] Swoole Redis[{$this->config['host']}:{$this->config['port']}]
