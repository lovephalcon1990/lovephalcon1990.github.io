...
class Timer implements IProcessData{
	...
	public function triger(){
		...
		foreach ($this->tickList as $tickId=>$tickInfo){
			...
			try {
				call_user_func($tickInfo['callback'], $tickInfo['data']);
			} catch (Throwable $th) {
				
			}
			$this->tickList['excuteTime'] += $this->tickList['interval'];
	 ...
	 * @return $tickId	定时id
	 */
	public function tick($callback, $data, $after=0, $repeatCount=1, $interval=0){
		$currentTime = time();
	...
	public function setStorageData($data){
		if(isset($data['tickList'])){
			$this->tickList = $data['tickList'];
		}
	}
